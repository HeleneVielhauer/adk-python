...
class PluginService:
  ...
  The PluginService is an internal class that orchestrates the invocation of
  ...
  def unregister_plugin(self, plugin_name: str) -> None:
    ...
    Args:
      plugin_name: The unique name of the plugin to unregister.

    Raises:
      ValueError: If no plugin with the given name is found.
    """
    matching_plugin = self.get_plugin(plugin_name)
    if not matching_plugin:
      raise ValueError(f"No plugin with name '{plugin_name}' is registered.")
    self.plugins.remove(matching_plugin)
    logger.info("Plugin '%s' unregistered.", plugin_name)
