...
from google.adk.plugins.plugin_service import PluginCallbackName
from google.adk.plugins.plugin_service import PluginService
import pytest
...
def service() -> PluginService:
  """Provides a clean PluginService instance for each test."""
...
def test_register_and_get_plugin(service: PluginService, plugin1: TestPlugin):
...
def test_register_duplicate_plugin_name_raises_value_error(
    service: PluginService, plugin1: TestPlugin
):
...
def test_unregister_plugin(service: PluginService, plugin1: TestPlugin):
  """Tests successful un-registration of a plugin."""
  service.register_plugin(plugin1)
  assert service.get_plugin("plugin1") is not None
...
async def test_early_exit_stops_subsequent_plugins(
    service: PluginService, plugin1: TestPlugin, plugin2: TestPlugin
):
...
async def test_plugin_exception_is_wrapped_in_runtime_error(
  ...
  """Tests that if a plugin callback raises an exception, the PluginService
